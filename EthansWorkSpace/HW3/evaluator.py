...
def get_position(node):
    ...
    if "line" in node:
        return node["line"]
    # for complex nodes, try index then base
    if node.get("tag") == "complex":
        idx = node.get("index")
        if isinstance(idx, dict):
            ...
            if "line" in idx:
                return idx["line"]
        return get_position(node.get("base"))
    return None
...
def evaluate(ast, environment, watchDebugger = None):
    if ast["tag"] == "number":
    ...
    if ast["tag"] == "list":
        items = []
        for item in ast["items"]:
            result, item_status = evaluate(item, environment, watchDebugger)
    ...
    if ast["tag"] == "object":
        object = {}
        for item in ast["items"]:
            key, key_status = evaluate(item["key"], environment, watchDebugger)
            if key_status == "exit": return key, "exit"
            assert type(key) is str, "Object key must be a string"
            value, value_status = evaluate(item["value"], environment, watchDebugger)
    ...
    if ast["tag"] == "identifier":
        ...
        if "$parent" in environment:
            return evaluate(ast, environment["$parent"], watchDebugger)
    ...
    if ast["tag"] == "+":
        left_value, l_status = evaluate(ast["left"], environment, watchDebugger)
        if l_status == "exit": return left_value, "exit"
        right_value, r_status = evaluate(ast["right"], environment, watchDebugger)
    ...
    if ast["tag"] == "negate":
        value, status = evaluate(ast["value"], environment, watchDebugger)
    ...
    if ast["tag"] == "assert":
        if ast["condition"]:
            condition_value, cond_status = evaluate(ast["condition"], environment, watchDebugger)
            ...
            if not is_truthy(condition_value):
                ...
                if "explanation" in ast and ast["explanation"]:
                    explanation_val, expl_status = evaluate(ast["explanation"], environment, watchDebugger)
    ...
    if ast["tag"] == "if":
        ...
        if is_truthy(condition_value):
            val, status = evaluate(ast["then"], environment, watchDebugger)
            if status: # Propagate "return", "exit", "break", "continue"
                return val, status
        else:
            if "else" in ast:
                val, status = evaluate(ast["else"], environment, watchDebugger)
                if status: # Propagate "return", "exit", "break", "continue"
                    return val, status
    ...
    if ast["tag"] == "while":
        ...
        while is_truthy(condition_value):
            val, body_status = evaluate(ast["do"], environment, watchDebugger)
    ...
    if ast["tag"] == "statement_list":
        ...
        for statement in ast["statements"]:
            last_value, status = evaluate(statement, environment, watchDebugger)
    ...
    if ast["tag"] == "program":
        last_value = None
        for statement in ast["statements"]:
            val, status = evaluate(statement, environment, watchDebugger)
            if status:
                if status == "return":
    ...
    if ast["tag"] == "call":
        function, func_status = evaluate(ast["function"], environment, watchDebugger)
        if func_status == "exit": return function, "exit"
        argument_values = []
        for arg in ast["arguments"]:
            arg_val, arg_status = evaluate(arg, environment, watchDebugger)
    ...
    if ast["tag"] == "complex":
        base, base_status = evaluate(ast["base"], environment, watchDebugger)
        if base_status == "exit": return base, "exit"
        index, index_status = evaluate(ast["index"], environment, watchDebugger)
    ...
    if ast["tag"] == "assign":
        ...
        value, value_status = evaluate(ast["value"], environment, watchDebugger)
        if watchDebugger and ast["target"].get("value") == watchDebugger:
            pos = get_position(ast["target"]) or "unknown"
            print(f"[WATCH] {ast['target']['value']} assigned to {value} at position {pos}")
        if value_status == "exit": return value, "exit"
    ...
    if ast["tag"] == "return":
        if "value" in ast and ast["value"] is not None: # Checks if 'return' has an expression
            evaluated_value, expression_status = evaluate(ast["value"], environment, watchDebuggers)
    ...
    if ast["tag"] == "exit":
        ...
        if "value" in ast and ast["value"] is not None:
            exit_code_val, status = evaluate(ast["value"], environment, watchDebugger)
    ...
    if ast["tag"] == "import":
        filename_val, status = evaluate(ast["value"], environment, watchDebugger)
        ...
        try:
            ...
            return evaluate(imported_ast, environment, watchDebugger) # Propagates value and status from imported code
