...
_watched_variable = None

def set_watched_variable(var_name):
    global _watched_variable
    _watched_variable = var_name

def log_watch(var_name, old_value, new_value, ast_node=None):
    if "position" in ast_node:
        location = f"position {ast_node['position']}"
    elif "target" in ast_node and "position" in ast_node["target"]:
        location = f"position {ast_node['target']['position']}"
    else:
        location = "unknown position"

    if _watched_variable is not None and var_name == _watched_variable:
        print(f"[WATCH] {var_name}: {old_value} -> {new_value} at {location}")
...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "assign":
        ...
        if isinstance(target_base, dict):
            old_value = target_base.get(target_index, "<undefined>")
        elif isinstance(target_base, list):
            if 0 <= target_index < len(target_base):
                old_value = target_base[target_index]
            else: 
                old_value = "<undefined>"
        else:
            old_value = "<undefined>"
        ...
        if _watched_variable is not None:
            if target["tag"] == "identifier":
                var_name = target_index
            else:
                var_name = ast_to_string(target)

            log_watch(var_name, old_value, value, ast)
