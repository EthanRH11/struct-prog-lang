...
def parse_simple_expression(tokens):
    ...
    if token["tag"] == "identifier":
        return {"tag": "identifier", "value": token["value"], "position": token.get("position")}, tokens[1:]

    if token["tag"] in {"boolean", "number", "string"}:
    ...
    while tokens[0]["tag"] in ["[", ".", "("]:
        ...
        if tokens[0]["tag"] == ".":
            ...
            ast = {
                "tag": "complex",
                "base": ast,
                "index": {"tag": "string", "value": tokens[0]["value"], "position": tokens[0].get("position")},
            }
            tokens = tokens[1:]
        if tokens[0]["tag"] == "(":
            tokens = tokens[1:]
